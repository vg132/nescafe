namespace Nescafe.Core
{
	...
	public abstract class Memory
	{
		...
		{
			for (var bytesRead = 0; bytesRead < size; bytesRead++)
			{
				var ReadAddr = (ushort)(address + bytesRead);
		...
		public ushort Read16WrapPage(ushort address)
		{
			...
			{
				var lo = Read(address);
				var hi = Read((ushort)(address & (~0xFF))); // Wrap around to start of page eg. 0x02FF becomes 0x0200
				data = (ushort)((hi << 8) | lo);
			}
			else
			{
